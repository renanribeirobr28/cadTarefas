...
$old_error_handler = set_error_handler("cadTarefas_scriptcase_error_handler");

function cadTarefas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['cadTarefas']['contr_erro']) && $_SESSION['scriptcase']['cadTarefas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_cadTarefas->contr_cadTarefas->Apl_com_erro = true;
            if ($inicial_cadTarefas->contr_cadTarefas->NM_ajax_flag)
            {
                $inicial_cadTarefas->contr_cadTarefas->NM_ajax_info['errList']['geral_cadTarefas'][] = $msg;
                return;
            }
            cadTarefas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function cadTarefas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['cadTarefas'])  && '' != $_SESSION['scriptcase']['error_icon']['cadTarefas'])  ? $_SESSION['scriptcase']['error_icon']['cadTarefas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['cadTarefas']) && '' != $_SESSION['scriptcase']['error_close']['cadTarefas']) ? $_SESSION['scriptcase']['error_close']['cadTarefas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['cadTarefas']) && '' != $_SESSION['scriptcase']['error_span_title']['cadTarefas']) ? $_SESSION['scriptcase']['error_span_title']['cadTarefas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['cadTarefas']) && '' != $_SESSION['scriptcase']['error_icon_title']['cadTarefas']) ? $_SESSION['scriptcase']['error_icon_title']['cadTarefas'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['cadTarefas']['error_buffer'] .= ob_get_flush();
}
